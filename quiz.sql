use mdl;
select _user.id as user_id,_course.id as course_id,_course.fullname as course, count(distinct _quiz.id) as total_quiz_course, count(distinct _quiz_attempts.quiz) as quiz_attemts,avg(_quiz_grade.grade) as avg_grad
