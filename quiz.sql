select _user.idnumber as user_id,_user.firstname,_user.lastname,
       ...
       count(distinct _quiz.id) as total_quiz_course,
       count(distinct _quiz_attempts.quiz) as quiz_attemts,
       avg(_quiz_grade.grade) as avg_grad
...
left join mdl_quiz_attempts as _quiz_attempts on _quiz_attempts.quiz = _quiz.id
                                                     and _quiz_attempts.userid=_user.id
left join mdl_quiz_grades as _quiz_grade on _quiz.id = _quiz_grade.quiz
                                                and _quiz_grade.userid = _user.id
