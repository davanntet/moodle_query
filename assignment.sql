select _user.id as user_id,_user.firstname,_user.lastname,_course.id as course_id,
       _course.fullname as course, count(distinct _assign.id) as total_assignment_course,
       count(distinct _assign_submission.assignment) as assignment_submission,
       avg(_assign_grade.grade) as avg_grad
...
left join mdl_assign_submission as _assign_submission on _assign_submission.assignment = _assign.id
                                                             and _assign_submission.userid=_user.id
left join mdl_assign_grades as _assign_grade on _assign.id = _assign_grade.assignment
                                                    and _assign_grade.userid = _user.id
