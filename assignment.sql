use mdl;
select _user.id as user_id,_user.firstname,_user.lastname,_course.id as course_id,_course.fullname as course, count(distinct _assign.id) as total_assignment_course, count(distinct _assign_submission.assignment) as assignment_submission,avg(_assign_grade.grade) as avg_grad
